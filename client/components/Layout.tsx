...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[hsl(264_100%_97%)] to-[hsl(264_100%_92%)] text-foreground">
      <header className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-background/70 bg-background/80 border-b">
        <div className="container flex h-16 items-center justify-between">
          <Link to="/" className="inline-flex items-center gap-2">
            ...
            <span className="font-semibold tracking-tight">Check‑In Invitados</span>
          </Link>
          {/* Reserve space for potential global actions */}
      ...
      <footer className="border-t py-6 text-center text-sm text-muted-foreground">
        © {new Date().getFullYear()} Check‑In Invitados
      </footer>
    </div>
  );
}
